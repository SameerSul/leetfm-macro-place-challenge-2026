...
```bash
PYTHONPATH=src \
uv run python test/diagnostic/_cuda_relocation_status.py \
...
```bash
PYTHONPATH=src \
uv run python test/verification/_verify_relocation_propose_all_auto.py

PYTHONPATH=src \
V2_RELOC_PROPOSE_ALL=auto V2_RELOC_PROPOSE_LOG=1 \
uv run python test/verification/_verify_relocation_propose_all.py
