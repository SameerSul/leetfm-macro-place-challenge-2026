...
> Trace playback supports 0.02× through 8×, including explicit 10× and 50×
> slower settings, with resumable play/pause from the selected event.
> Replay-only H.264 MP4 export captures the full dashboard at the selected
> speed through ImageIO/FFmpeg and uses atomic output replacement; it cannot
> back-pressure a live placement worker.
> Slow exports encode each state once at a proportionally lower frame rate and
> the CLI reports percentage and ETA, avoiding silent duplicate-frame bloat.
> `--replay` accepts an explicit JSONL file or selects the newest JSONL in a
> supplied directory; invalid paths now fail argument validation without a Qt
> traceback.
...
> completion placement. All **16 visualizer tests** pass, including offscreen
...
> project suite is **110 passed, 2 GPU-only skipped**. Live-run
...
52. [~] **Dataflow-corridor seed alternatives** -- TESTED AND REJECTED 2026-07-20. Added two deterministic rigid cluster perturbations from weighted inter-cluster low-fanout edges, with owned-soft co-movement and the existing legalization/contract/exact gates. The candidates never selected on the 17-design IBM sweep; all exact placements and scores remained unchanged at `AVG 1.1404`, `17/17 VALID`, zero overlaps. The added prescore work had no measured quality benefit, so the candidates were removed.
53. [~] **Legalization sequence seed alternatives** -- TESTED AND REJECTED 2026-07-20. Added area-first and reverse-flow legalization orders as deterministic DREAMPlace seed candidates, retaining the normal safety legalization and all existing exact gates. Neither sequence candidate was selected on the 17-design IBM sweep; all placements and scores remained unchanged at `AVG 1.1404`, `17/17 VALID`, zero overlaps. The candidates were removed.
54. [~] **Inferred route-channel seed alternative** -- TESTED AND REJECTED 2026-07-20. Broadened the existing explicit-tag route-channel perturbation to inferred flat IBM clusters. It passed all legalization and hierarchy gates but never won seed selection; the full IBM sweep remained `AVG 1.1404`, `17/17 VALID`, zero overlaps. The broader candidate was removed because its prescore overhead produced no quality gain.
55. [~] **Contract-slack proxy sleeve** -- TESTED AND REJECTED 2026-07-20. Added a bounded, contract-checked soft-relocation lane using remaining round headroom as a proxy-improvement sleeve. The full IBM sweep stayed `17/17 VALID` with zero overlaps, but regressed from `AVG 1.1404` to `1.1433` in `387.07s`; the lane was removed. The verification suite remained green (`77 passed`).
56. [~] **Dataflow-aware soft target ranking** -- TESTED AND REJECTED 2026-07-20. Blended each soft macro's exact net-connection centroid into target ranking across the soft-relocation lanes, while preserving all hierarchy and exact-proxy gates. The full IBM sweep remained `17/17 VALID` with zero overlaps, but reached `AVG 1.1414` versus the `1.1404` reference in `321.39s`; the proposal-order change was removed. All 77 verification tests passed.
