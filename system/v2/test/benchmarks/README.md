...
`testcases/`, `processed/`, and `vis/` are **not committed** (~140MB total;
see the local `.gitignore`). Regenerate everything with the two commands at
the top of Usage - generation is deterministic, so the rebuilt suite is
identical.
