...
Runs the hierarchy-floorplan mode (V2_HIER_FLOORPLAN=1) with region relief OFF
then ON, and reports final-placement connectivity closeness (hard-hard / intra /
hard-soft, as fraction of canvas diagonal) plus exact proxy. Expectation: proxy
DROPS while closeness stays near the OFF (floorplan) values, i.e. congestion is
relieved without sending macros far from their cluster.
...
def _run(bench, relief):
    ...
    os.environ["V2_HIER_REGION_RELIEF"] = "1" if relief else "0"
    ...
    return {"proxy": proxy, "hh": _dist(pos, hh, diag),
            "intra": _dist(pos, intra, diag), "hs": _dist(pos, hs, diag)}
...
if __name__ == "__main__":
    ...
    for b in benches:
        ...
        print(f"  relief OFF   {off['proxy']:8.4f} {off['hh']:8.4f} {off['intra']:8.4f} {off['hs']:8.4f}")
        print(f"  relief ON    {on['proxy']:8.4f} {on['hh']:8.4f} {on['intra']:8.4f} {on['hs']:8.4f}")
        print(f"  delta        {on['proxy']-off['proxy']:+8.4f} {on['hh']-off['hh']:+8.4f} "
