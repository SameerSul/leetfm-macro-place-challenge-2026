...
- `results.json` - per-benchmark cost breakdown (initial vs placed, deltas,
  validity, out-of-bounds details), generator-truth hierarchy contract, and
...
The generator stores its hard/soft community assignments in metadata. The
runner evaluates the final placement against those labels, reports hard-cluster
coverage, majority purity, and pair precision/recall for production inference,
and emits `hierarchy_truth_audit` telemetry. Truth limits are relative to the
generated seed, not the production pipeline's legalized inferred-hierarchy
reference; `reference_overlaps` identifies cases where that distinction
...
- Generated by `generate_benchmarks.py` and gitignored:
  ...
  - `metadata/<name>.json` - generation config, axis description, and hard/soft
    hierarchy truth labels per benchmark
