"""Check the production swap masks against their NumPy reference."""
...
def test_numba_hard_hard_matches_numpy_legality(monkeypatch):
    ...
    monkeypatch.setattr(hierarchy_swaps, "HAS_NUMBA", False)
...
def test_numba_hard_soft_matches_numpy_legality(monkeypatch):
    hard_pos, sizes, hw, hh = _hard_fixture()
    soft_pos = np.array(
