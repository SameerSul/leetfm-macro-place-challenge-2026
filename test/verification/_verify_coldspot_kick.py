...
Builds a real congestion field, then calls `_coldspot_cluster_kick_candidates` across many
...
def _check(bench):
    ...
    for s in range(25):
        ...
        for pk in ("hot", "random"):
            res = _coldspot_cluster_kick_candidates(
                hard_xy,
                sizes[:n],
                hw,
                hh,
