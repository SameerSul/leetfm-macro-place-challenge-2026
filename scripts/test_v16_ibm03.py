"""
v16 ibm03 test: n=290, grid=32x29=928 cells, t_score=9-12s.
v14 best=1.3547 (vs RePlAce=1.3222, -2.5%). 3300s -> ~220+ noise restarts.
adaptive frac=0.01 at iter=7-8 wins in v14; expect improvement with more restarts.

Run from project root: python scripts/test_v16_ibm03.py
"""
import sys, time
...
benchmark_dir = "external/MacroPlacement/Testcases/ICCAD04/ibm03"
print("[ibm03 v16] Loading benchmark...", flush=True)
...
print("[ibm03 v16] Running placer.place()...", flush=True)

t0 = time.time()
placement = placer.place(benchmark)
t_place = time.time() - t0
print(f"[ibm03 v16] place() done in {t_place:.1f}s", flush=True)

costs = compute_proxy_cost(placement, benchmark, plc)
result = costs['proxy_cost']
print(f"[ibm03 v16] Final proxy={result:.4f}", flush=True)
print(f"  wl={costs['wirelength_cost']:.3f} den={costs['density_cost']:.3f} "
      f"cong={costs['congestion_cost']:.3f}", flush=True)
print(f"[ibm03 v16] Total: {time.time()-t0:.1f}s", flush=True)
