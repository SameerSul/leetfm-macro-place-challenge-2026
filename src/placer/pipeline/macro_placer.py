    ...
    def _hierarchy_floorplan(self, benchmark: Benchmark) -> "torch.Tensor | None":
        ...
        def _record_plateau(
            ...
            **extra,
            ...
            if const.HIER_PLATEAU_TELEMETRY:
        ...
        def _is_plateau(record: PlateauTelemetry | None) -> bool:
            if record is None or not const.HIER_BUDGET_AWARE_SCHEDULING:
                return False
        ...
        if const.HIER_REGION_RELIEF:
            ...
            if const.HIER_STRONG_SOFT_REPAIR and n_soft and bool(np.any(soft_mov)):
                ...
                plateau_soft_bonus = bool(
                    const.HIER_PLATEAU_SOFT_REPAIR_BONUS
                    and plateau_trigger
                    and _has_spare(
                        rdeadline,
                ...
                scheduled_strong_budget = strong_budget + (
                    float(const.HIER_PLATEAU_SOFT_REPAIR_BONUS_BUDGET_S)
        ...
        if const.HIER_COLDSPOT_KICK:
            ...
            ck_graph_select = bool(const.HIER_COLDSPOT_GRAPH_SELECT)
            if ck_graph_select and not (ck_gnn_select or ck_oracle):
                ck_gnn_kicks = max(
                    ck_gnn_kicks,
            ...
            def _expand_bbox_to_adjacent_cold(
                ...
                if not bool(const.HIER_COLDSPOT_ADAPTIVE_REGIONS):
            ...
            def _coldspot_local_regions(
                ...
                min_pad = max(cell_w, cell_h) * max(
                    0.0, float(const.HIER_COLDSPOT_LOCAL_MIN_PAD_CELLS)
                )
                ...
                xhi = min(cw, xhi + pad)
                yhi = min(ch, yhi + pad)
                bbox_mask = _bbox_cell_mask(xlo, ylo, xhi, yhi)
                if not bool(const.HIER_COLDSPOT_GRAPH_MASK_GATING):
                    region_mask = bbox_mask
                target_mask = region_mask & ~_occupied_cells(hard_xy, soft_xy)
                if bool(const.HIER_COLDSPOT_GRAPH_TARGET_POOL) and open_cold_mask.any():
                ...
                soft_region = np.column_stack(
                    [soft_hw, soft_hh, cw - soft_hw, ch - soft_hh]
                ).astype(np.float64)
                for k in local_soft:
                    soft_region[k] = (
            ...
            def _refine_coldspot_candidate(
                ...
                if not bool(const.HIER_COLDSPOT_LOCAL_REFINE):
                ...
                fields = (False, True) if const.HIER_SWAP_DENSITY_FIELD else (False,)
                for use_density in fields:
                ...
                refined_h, got, score = _relocation_moves(
                    ...
                    target_pool=(
                        local_target_pool if bool(const.HIER_COLDSPOT_GRAPH_TARGET_POOL) else None
                    ),
                    region_mask=(
                        local_region_mask if bool(const.HIER_COLDSPOT_GRAPH_MASK_GATING) else None
                    ),
                )
                ...
                if local_s_mask.any():
                    refined_s, got, score = _soft_relocation_moves(
                        ...
                        target_pool=(
                            local_target_pool
                            if bool(const.HIER_COLDSPOT_GRAPH_TARGET_POOL)
                            else None
                        ),
                        region_mask=(
                            local_region_mask
                            if bool(const.HIER_COLDSPOT_GRAPH_MASK_GATING)
                            else None
                        ),
                    )
            ...
            for _ in range(ck_rounds):
                ...
                if include_noop:
                    ...
                    ck_candidate_id += 1
                for rank, (kh, ks, ck_trace) in enumerate(
                ...
                for cand in candidate_records:
                    log_gnn_event(
                        ...
                        selector_top_k=int(
                            ck_gnn_top_k
                            if ck_gnn_select
                            else (ck_graph_top_k if ck_graph_select else len(candidate_records))
                        ),
                        selected_by_gnn=bool(ck_gnn_select and id(cand) in selected_ids),
                        graph_selector_enabled=bool(ck_graph_select and not ck_gnn_select),
                        selected_by_graph=bool(
                            ck_graph_select and not ck_gnn_select and id(cand) in selected_ids
                        ),
            ...
            if (
                bool(const.HIER_COLDSPOT_GRAPH_FALLBACK)
                and ck_acc == 0
                ...
                committed_fallback_id = (
                    int(accepted_fallback["candidate_id"])
            ...
            elif hier_post_coldspot_micro and ck_skip_micro:
        ...
        legality_margin = (
            _hard_legality_margin(legal, margin_eps)
            if const.HIER_LEGALITY_MARGIN_AUDIT
            else {"pair_margin": 0.0, "bounds_margin": 0.0, "min_margin": 0.0}
        )
        log_gnn_event(
            "hier_final",
            benchmark=benchmark.name,
            ...
            hierarchy_oversize_split=bool(const.HIER_OVERSIZE_CLUSTER_SPLIT),
            hierarchy_split_parents=int(len(hierarchy.split_parents)),
            seed_portfolio=bool(const.HIER_SEED_PORTFOLIO),
            ...
            legality_margin_audit=bool(const.HIER_LEGALITY_MARGIN_AUDIT),
        ...
        _log(
            ...
            f"oversize={int(bool(const.HIER_OVERSIZE_CLUSTER_SPLIT))}, "
            f"seed={selected_seed_name}, "
            f"additive={int(bool(const.HIER_ADDITIVE_CANDIDATE_POOLS))}, "
