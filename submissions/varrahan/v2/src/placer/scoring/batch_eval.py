"""S2 — batched candidate evaluation for hard-macro relocation.
...
Selection uses these scores to pick the best candidate; the commit still goes
through `IncrementalScorer._commit_after_prep` (exact), and the R2 round gates on
a true `_exact_proxy`, so ~1e-12 batched drift cannot cause a regression.

STATUS: explored end-to-end, verified, but NOT a win on the IBM grids — kept as
correct, isolated artifacts (zero production impact). Variants, all bit-exact vs
sequential `_trial_at` (`_verify_batch_eval.py`, Δ ≤ 4.4e-16):
...
The GPU compute ceiling is ~12× but unreachable at this granularity. These would
win on much larger grids. See ARCHITECTURE.md §6.2.
"""
...
def _score_candidates_hard(sc, prep, cand_xy):
    ...
    # --- cong + density (per-candidate fill on the shared base; reductions could
    #     batch on GPU next — kept correct + simple here) ---
...
# ---------------------------------------------------------------------------
# GPU batched smooth + congestion (the production path; matches the reference)
# ---------------------------------------------------------------------------
